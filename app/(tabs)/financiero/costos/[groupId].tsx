/**
 * /financiero/costos/[groupId] — Level-2 terceros screen for a COGS group.
 *
 ...
 *   - Scrollable: search bar + MlClientRow list (first row selected + outlined)
 */
...
export default function CostosGrupoTercerosScreen() {
  ...
  const items = useMemo(
    () => normalizePnLSection(pnl.data ?? null, "COGS"),
    [pnl.data],
  );
  ...
  /** Bar chart: one bar per tercero (placeholder) */
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <MlEmptyState
          icon="search-off"
          title="Sin costos en este periodo"
          description={
            activePeriodKey === "12m"
        ...
        <View className="gap-4">
          ...
          {/* Tercero rows — first row selected + outlined */}
          <View className="gap-1">
            {filteredTerceros.map((t, i) => {
