...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { datasetId, daxQuery, groupId } = (await req.json()) as {
      datasetId?: string;
      daxQuery?: string;
      groupId?: string;
    };
    if (!datasetId || !daxQuery) {
      return respond("datasetId + daxQuery required", { status: 400 });
    }
    ...
    if (!pbiRes.ok) {
      ...
      return respond(`PBI ${pbiRes.status}: ${txt}`, { status: 502 });
    }
