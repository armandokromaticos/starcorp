-- Revert get_client_cost_groups to flat per-nombre_cuenta shape (0014 behavior).
-- A previous experiment (never landed in this file tree) added a hierarchical
-- jsonb `accounts` column; this migration drops it so consolidado costos/gastos
-- screens go back to a one-row-per-account list that navigates directly to terceros.

drop function if exists get_client_cost_groups(text, text, text, boolean);
