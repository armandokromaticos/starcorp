...
export function OrClientDetail({ clientId, entryCategory }: OrClientDetailProps) {
  ...
  return (
    ...
    >
      {/* Pinned top: search + breadcrumb + period filter */}
      <View className="gap-4 pt-2 pb-3 bg-bg-secondary">
        <View className="px-4">
      ...
      >
        ...
        {metaQuery.isPending && !meta ? (
          ...
          <View className="flex-row gap-3 px-4">
            ...
            <MlStatBox
              icon="receipt-long"
              value={formatCompact(meta.nEntries)}
              label="Movimientos"
              iconColor="#22D3EE"
              variant="dark"
            />
            <MlStatBox
              icon="event"
              value={meta.firstFecha ? `Desde ${meta.firstFecha.slice(0, 4)}` : '—'}
              label="Vigencia"
              iconColor="#A78BFA"
              variant="dark"
            />
