/**
 * Organism: OrCarteraFiltersSheet
 *
 * Bottom sheet con filtros del Informe Cartera. Permite buscar
 * clientes, ver los seleccionados como chips removibles, y aplicar.
 */
...
interface OrCarteraFiltersSheetProps {
  visible: boolean;
  onClose: () => void;
  clients: ClientOption[];
  initialFilters: CarteraFilters;
  onApply: (filters: CarteraFilters) => void;
}

export const OrCarteraFiltersSheet = memo<OrCarteraFiltersSheetProps>(
  ({ visible, onClose, clients, initialFilters, onApply }) => {
    const insets = useSafeAreaInsets();
    const [mounted, setMounted] = useState(visible);
    const [search, setSearch] = useState('');
    const [selectedIds, setSelectedIds] = useState<string[]>(
      initialFilters.clientIds,
    );
    ...
    return (
      ...
      >
        <View
          style={{ flex: 1, pointerEvents: visible ? 'auto' : 'none' }}
        >
          {visible && (
            <Animated.View
              key="sheet-overlay"
            ...
            >
              ...
              <View className="flex-row justify-between items-center px-6 pb-3">
                <AtTypography variant="caption" color="#4A5568">
                  Filtros aplicados: {selectedIds.length}
                </AtTypography>
              ...
              >
                ...
                <View className="gap-2">
                  <AtTypography variant="bodyBold">Por cliente</AtTypography>

                  <Pressable
                    onPress={() => setDropdownOpen((o) => !o)}
                    className="flex-row items-center justify-between rounded-full bg-bg-secondary px-4 py-3"
                    style={{ borderCurve: 'continuous' }}
                  >
...
OrCarteraFiltersSheet.displayName = 'OrCarteraFiltersSheet';
